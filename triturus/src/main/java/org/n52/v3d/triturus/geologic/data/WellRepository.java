...
/**
 * Repository to manage a set of wells data objects. Basically, well information tuples &lt;<i>well</i>, <i>loc</i>, <i>kb</i>,
 * <i>md</i>&gt; consisting of
 * <ul>
 ...
 * <li>marker depth <i>depth</i> referring to the ground level + Kelly bushing elevation, if set, see
 * {@link Well#setKellyBushingHeight(double)}, and</li>
...
public class WellRepository {
    ...
    public void addWell(String wellName, VgPoint loc, Double kb, Double maxDepth) {
        ...
        Well w = this.getWellEntry(wellName, loc);
    ...
    /**
     * adds a marker information to the repository. Note that depth values indicate locations below ground (&quot;-z direction&quot;). The
     * marker depth is given with respect to the well's ground level
     * <tt>w.groundLevel()</tt> + KB, where KB gives the Kelly bushing height, see {@link setKellyBushingHeight}.
     *
     * @param wellName Well name (unique identifier)
    ...
    public void addMarker(String wellName, VgPoint pos, Double depth, String markerName) {
        // Check, if well feature is already in repository:
        Well w = this.getWellEntry(wellName, pos);
        // If not, then create a new well object:
        if (w == null) {
            w = new Well(wellName);
            w.setLocation(pos);
     ...
     * @return Well object
     */
    private Well getWellEntry(String wellName, VgPoint pos) {
        if (wells.size() <= 0 || wellName == null || pos == null) {
            return null;
        }
        int i = 0;
        while (i < wells.size()) {
            if (wellName.equals(wells.get(i).getName()) && wells.get(i).getLocation().getX() == pos.getX() && wells.get(i).getLocation().getY() == pos.getY()) {
                return wells.get(i);
            }
            i++;
        }
